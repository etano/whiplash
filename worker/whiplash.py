import sys, logging, json, time, zlib, os, getpass
...
class collection:
    ...
    def request(self, protocol, uri, payload):
        ...
        t1 = time.time()
        logging.debug("%r collection: request: protocol: %r, uri: %r, payload: %r" % (t1-t0, protocol, uri, payload))
        return response
...
class db:
    ...
    def query(self, filters, fields={}, settings={}):
        ...
        t1 = time.time()
        logging.debug("%r query: filters: %r, fields: %r, settings: %r" % (t1-t0, filters, fields, settings))
        return res
    ...
    def status(self, filters={}):
        ...
        t1 = time.time()
        logging.debug("%r status: filters: %r" % (t1-t0, filters))
        return res
    ...
    def submit(self, filters, settings={}):
        ...
        t1 = time.time()
        logging.debug("%r submit: filters: %r, settings %r" % (t1-t0, filters, settings))
        return res
