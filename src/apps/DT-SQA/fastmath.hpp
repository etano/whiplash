/* Copyright (c) Troels F. Roennow, ETH Zurich */
#ifndef __FASTEXP_HPP__
#define __FASTEXP_HPP__
#include <math.h>
#include <stdlib.h>
#include <cassert>
...
namespace fastmath {  
  ...
  template< bool handle_overflow = false, size_t n = 8, typename data = __detail__::exp_correction<n>,  typename type_t = double, typename int_t = int32_t,  size_t significant_bits = 20, int c = 60801, size_t little_edian = 1>
  class exp {
    int_t a, b;
    enum { mask = (1 << n) - 1  };
    int_t upper_limit, lower_limit, sign, first_sign;
    union { int_t uval[2]; type_t dval; } bounds;
  public:
    /**
    ...
    /**
       @brief computes exp(c*x) where c = 1 if not changed by set_coefficient.
    ...
    union { int_t uval[2]; type_t dval; } y;
      ...
      if(n != 0)   y.dval *=  data::data[ mask & (tmp >> (significant_bits - n ) )  ];
      return y.dval ;
    }
  };


  template< typename data,  typename type_t, typename int_t,  size_t significant_bits, int c, size_t little_edian >
  ...
  template< size_t n = 8, typename data = __detail__::exp_correction<n>,  typename type_t = double, typename int_t = uint32_t,  size_t significant_bits = 20, int c = -1, size_t little_edian = 1>
  class log {
    type_t a, b, ia;    
    enum { mask = (1 << n) - 1 };
  ...
  public:
    ...
    inline void set_coefficient(double const& coef = 1) { a = type_t(1<< significant_bits )/M_LN2*coef; ia = 1./a; }
