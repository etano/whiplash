...
namespace wdb { namespace entities { namespace factory {
    ...
    std::unique_ptr<generic::model> make_model(std::string class_name, Params&&... parameters) {
        ...
        switch(class_id){
            case 1 : return std::unique_ptr<ising::model>(new ising::model(class_name, parameters...));
            case 2 : return std::unique_ptr<tsp::model>(new tsp::model(class_name, parameters...));
            case 3 : return std::unique_ptr<qubo::model>(new qubo::model(class_name, parameters...));
            case 4 : return std::unique_ptr<sat::model>(new sat::model(class_name, parameters...));
        }
    ...
    std::unique_ptr<generic::property> make_property(std::string class_name, Params&&... parameters) {
        int class_id = class_id_by_name[class_name];
        switch(class_id){
            case 1 : return std::unique_ptr<ising::property>(new ising::property(class_name, parameters...));
            case 2 : return std::unique_ptr<tsp::property>(new tsp::property(class_name, parameters...));
            case 3 : return std::unique_ptr<qubo::property>(new qubo::property(class_name, parameters...));
            case 4 : return std::unique_ptr<sat::property>(new sat::property(class_name, parameters...));
        }
        printf("Hey model class id is %d\n", class_id);
        throw std::runtime_error("Unknown model class");
    }

    std::unique_ptr<entities::generic::property> make_property(odb::iobject& o){
        std::string model_class = entities::generic::reader::String(o, "class");
