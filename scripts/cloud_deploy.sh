...
ssh ec2-user@${ec2_host} 'sudo yum install -y docker gcc-c++ boost lapack'
scp -r udyn ec2-user@${ec2_host}:
ssh ec2-user@${ec2_host} 'wget http://sourceforge.net/projects/boost/files/boost/1.60.0/boost_1_60_0.tar.bz2 && tar -xjvf boost_1_60_0.tar.bz2 && cd udyn && make'
