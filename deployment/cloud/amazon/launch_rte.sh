...
docker run -d -v /var/run/docker.sock:/var/run/docker.sock -v /usr/bin/docker:/bin/docker -v $PWD:/input -e "WORKDIR=$PWD" -p 1337:1337 whiplash/rte sh -c "./rte/manager.py --cloud --host ${APIIP} --port 1337 --verbose --docker"
