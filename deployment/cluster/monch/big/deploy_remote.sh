...
do
    sh deployment/cluster/monch/big/remote_command.sh ${machines[${i}]} "killall mongod; killall mongos"
done
...
#sh deployment/cluster/monch/big/remote_command.sh ${main_machine} "rm -rf ${data_dir} ${log_dir}" #WARNING: debug

sh deployment/cluster/monch/big/remote_command.sh ${main_machine} "mkdir -p ${data_dir}; mkdir -p ${log_dir}; openssl rand -base64 741 > ${key_file}; chmod 600 ${key_file}"

#deploy servers

echo "Deploying servers"

sh deployment/cluster/monch/big/remote_command.sh ${main_machine} "mkdir -p ${server_log_dir}; mkdir -p ${server_data_dir}"

#set access control

server_data_dir_n=${server_data_dir}/db_0

sh deployment/cluster/monch/big/remote_command.sh ${server_hosts[0]} '
...
do
    ...
    sh deployment/cluster/monch/big/remote_command.sh ${server_hosts[${i}]} '
    mkdir -p '"${server_data_dir_n}"';
    echo "
processManagement:
 fork: true
...
sh deployment/cluster/monch/big/remote_command.sh ${main_machine} "mkdir -p ${router_log_dir}"

for (( i=0; i<${#router_hosts[@]}; i++ ))
do
sh deployment/cluster/monch/big/remote_command.sh ${router_hosts[${i}]} '
echo "
processManagement:
 fork: true
systemLog:
...
sh deployment/cluster/monch/big/remote_command.sh ${main_machine} "mkdir -p ${shard_log_dir}; mkdir -p ${shard_data_dir}"
...
do
    ...
    sh deployment/cluster/monch/big/remote_command.sh ${replica_hosts[${ind0}]} '
...
'
    ...
    do
        ...
        sh deployment/cluster/monch/big/remote_command.sh ${replica_hosts[${ind}]} '
        mkdir -p '"${replica_data_dir_n}"';
        echo "
processManagement:
 fork: true
